...
import { BarChart3, Fuel, TrendingUp, Package, Bell, ChevronDown, ChevronUp } from "lucide-react";

const FuelDistribution = ({ fuelDistribution, isLoadingMaster, c1RemainingFuelByCategory, c6RemainingFuelByCategory, c1TotalRemainingFuel, c6TotalRemainingFuel }) => {
  ...
  const totalFuel = (fuelDistribution.c1RemainingFuel || 0) + (fuelDistribution.c6RemainingFuel || 0);
  const c1Percentage = totalFuel > 0 ? ((fuelDistribution.c1RemainingFuel / totalFuel) * 100).toFixed(1) : 0;
  const c6Percentage = totalFuel > 0 ? ((fuelDistribution.c6RemainingFuel / totalFuel) * 100).toFixed(1) : 0;
  ...
  const hasLowFuelInC1 = !isLoadingMaster && c1RemainingFuelByCategory.some(item => item.remainingFuel < 500);
  const hasLowFuelInC6 = !isLoadingMaster && c6RemainingFuelByCategory.some(item => item.remainingFuel < 500);
  const isC1LowRegion = !isLoadingMaster && fuelDistribution.c1RemainingFuel < 2000;
  const isC6LowRegion = !isLoadingMaster && fuelDistribution.c6RemainingFuel < 2000;
    ...
    <div className="bg-gradient-to-br from-[#1a2142] to-[#13182b] rounded-2xl border border-gray-700 shadow-xl overflow-hidden flex flex-col">
      ...
      <div className="bg-[#0a0f20] px-5 py-3 border-b border-gray-700 flex-shrink-0">
        <div className="flex items-center justify-between">
          ...
          <div className="relative">
            <Bell size={16} className="text-yellow-400 cursor-pointer hover:text-yellow-300 transition-colors" />
            {(hasLowFuelInC1 || hasLowFuelInC6) && <NotificationBadge count={1} />}
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="border-b border-gray-700/50">
            ...
            <div className="bg-gradient-to-r from-blue-950/40 to-[#0f1325]/70 rounded-xl p-4 border-l-4 border-blue-500 relative">
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-200 text-sm font-medium">C-1 Region</span>
                </div>
                {isLoadingMaster ? (
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-[10px] text-gray-500">Liters remaining</span>
                  </div>
                )}
              </div>
              {!isLoadingMaster && (
          ...
          >
            <div ref={c1TableRef}>
              {!isLoadingMaster && c1RemainingFuelByCategory && c1RemainingFuelByCategory.length > 0 && (
                <div className="px-4 pb-4">
                  <div className="bg-[#0f1325]/50 rounded-xl overflow-hidden">
                    <div className="px-3 py-2 border-b border-gray-700 bg-blue-500/5">
                      <p className="text-xs font-medium text-blue-400">Remaining Fuel by Site Category</p>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="bg-[#0f1325]">
                          <tr>
                            <th className="p-2 text-left text-gray-400 font-medium text-[10px]">Category (Site Type)</th>
                            <th className="p-2 text-right text-gray-400 font-medium text-[10px]">Remaining Fuel (L)</th>
                            <th className="p-2 text-right text-gray-400 font-medium text-[10px]">% of Total</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-800">
                          {c1RemainingFuelByCategory.map((item, idx) => {
                            const percentage = getPercentage(item.remainingFuel, c1TotalRemainingFuel);
                            const isLowFuel = item.remainingFuel < 500;
                            return (
                              <tr key={idx} className={`hover:bg-blue-500/5 transition-colors ${isLowFuel ? 'bg-red-500/5' : ''}`}>
                                <td className="p-2">
                                  <div className="flex items-center gap-1">
                                    ...
                                    {isLowFuel && (
                                      <span className="text-[8px] bg-red-500/20 text-red-400 px-1 rounded">Low</span>
                                    )}
                                  </div>
                                </td>
                                <td className="p-2 text-right">
                                  <span className={`text-xs font-mono ${isLowFuel ? 'text-red-400 font-bold' : 'text-white'}`}>
                                ...
                                <td className="p-2 text-right">
                                  <div className="flex items-center justify-end gap-1">
                                    ...
                                    <span className="text-[10px] text-gray-400 w-8">{percentage}%</span>
                                  </div>
                                </td>
                              </tr>
                            );
                        ...
                        <tfoot className="bg-[#0f1325] border-t border-gray-700">
                          <tr>
                            ...
                            <td className="p-2 text-right text-white font-bold text-xs">{c1TotalRemainingFuel.toLocaleString()} L</td>
                            <td className="p-2 text-right text-white font-bold text-xs">100%</td>
                          </tr>
                        </tfoot>
                      </table>
                    </div>
        ...
        <div>
            ...
            <div className="bg-gradient-to-r from-purple-950/40 to-[#0f1325]/70 rounded-xl p-4 border-l-4 border-purple-500 relative">
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-gray-200 text-sm font-medium">C-6 Region</span>
                </div>
                {isLoadingMaster ? (
          ...
          >
            <div ref={c6TableRef}>
              {!isLoadingMaster && c6RemainingFuelByCategory && c6RemainingFuelByCategory.length > 0 && (
                <div className="px-4 pb-4">
                  <div className="bg-[#0f1325]/50 rounded-xl overflow-hidden">
                    <div className="px-3 py-2 border-b border-gray-700 bg-purple-500/5">
                      <p className="text-xs font-medium text-purple-400">Remaining Fuel by Site Category</p>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="bg-[#0f1325]">
                        ...
                        <tbody className="divide-y divide-gray-800">
                          {c6RemainingFuelByCategory.map((item, idx) => {
                            const percentage = getPercentage(item.remainingFuel, c6TotalRemainingFuel);
                            const isLowFuel = item.remainingFuel < 500;
                            return (
                              <tr key={idx} className={`hover:bg-purple-500/5 transition-colors ${isLowFuel ? 'bg-red-500/5' : ''}`}>
                                <td className="p-2">
                                  <div className="flex items-center gap-1">
                                    <span className={`text-xs font-medium ${getCategoryColor(item.category)}`}>
                                      {item.category}
                        ...
                        <tfoot className="bg-[#0f1325] border-t border-gray-700">
                          <tr>
                            <td className="p-2 text-white font-bold text-xs">Total</td>
                            <td className="p-2 text-right text-white font-bold text-xs">{c6TotalRemainingFuel.toLocaleString()} L</td>
                            <td className="p-2 text-right text-white font-bold text-xs">100%</td>
                          </tr>
                        </tfoot>
                      </table>
        ...
        {!isLoadingMaster && totalFuel > 0 && (
          <div className="p-4 pt-2 border-t border-gray-700/50 mt-auto">
            <div className="flex justify-between items-center">
              <span className="text-xs text-gray-400">Total Remaining Fuel</span>
              <span className="text-base font-bold text-cyan-400">{totalFuel.toLocaleString()} L</span>
            </div>
          </div>
        )}
      </div>
