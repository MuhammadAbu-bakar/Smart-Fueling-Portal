        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 text-xs mt-1">
              Based on Severity & Remaining Fuel
            </p>
          </div>
          <button
            onClick={onExport}
          ...
          <tbody className="divide-y divide-gray-800">
            {isLoadingMaster ? (
              ...
              dgCategories.map((cat, idx) => {
                const percentage =
                  totalDGs > 0 ? ((cat.total / totalDGs) * 100).toFixed(1) : 0;
                ...
                return (
                  <tr
                    key={idx}
                    className={`hover:bg-white/5 transition-colors ${bgClass}`}
                  >
                    <td className="p-3 text-white font-medium">
                      {cat.name}
                      {cat.total > 0 && (
                        <span className={`${colorClass} text-xs ml-2`}>
                          ({percentage}%)
                        </span>
                      )}
                    </td>
                    ...
                    <td className="p-3 text-right text-red-400 font-mono">
                      {cat.less50.toLocaleString()}
                    </td>
                    <td className="p-3 text-right text-green-400 font-mono">
                      {cat.greater50.toLocaleString()}
                    </td>
      ...
      {!isLoadingMaster && totalDGs === 0 && (
        <div className="p-8 text-center text-gray-400">
          ...
          <p className="text-xs mt-2">
            Please check that Column S contains values like: PTN Node, Critical
            Hub (10 ++), Major Hub (5~10), Minor Hub (1~4), or Single/FTTS
          </p>
        </div>
      )}
    </div>
