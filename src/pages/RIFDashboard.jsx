...
const RIFDashboard = () => {
  ...
  // Parse coordinates from Google Sheets format (Column R)
  ...
  // Get weather risk from Open-Meteo API
  const getWeatherRiskFromAPI = async (latitude, longitude) => {
    try {
      ...
      const precipProbability = data.daily?.precipitation_probability_max?.[0] || 0;
      const windSpeed = data.current_weather?.windspeed || data.daily?.windspeed_10m_max?.[0] || 0;
      ...
      return { risk, weatherDesc, precipProbability, windSpeed, temperature: data.current_weather?.temperature };
      
    } catch (error) {
      console.error("Weather API error:", error);
      return { risk: "Low", weatherDesc: "Unknown", precipProbability: 0, windSpeed: 0, temperature: null };
    }
  };

  // Fetch weather for all sites with rate limiting
  const fetchWeatherForSites = async (sites) => {
    ...
    for (let i = 0; i < sites.length; i += concurrencyLimit) {
      ...
      const batchPromises = batch.map(async (site) => {
        if (site.coordinates) {
          const weather = await getWeatherRiskFromAPI(site.coordinates.lat, site.coordinates.lng);
  ...
  const exportToExcel = () => {
    const currentFilteredAlarms = rifAlarms.filter(alarm => {
      const regionMatch = selectedRegion === "All" || alarm.region === selectedRegion;
      return regionMatch;
    });
    ...
    const headers = [
      "Site Name", "Site ID", "Region", "Duration", "Occur Time",
      "Alarm Message", "Remaining Fuel (L)", "Weather Risk", "Weather Description",
      "Precipitation (%)", "Wind Speed (km/h)", "Temperature (°C)", "Category"
    ];
  ...
  const fetchRIFData = async () => {
    ...
    try {
      ...
      for (const row of dataRows) {
        const rectifierAlarm = row[10]; // Column K - Rectifier Input Failure
        
        const hasRIF = rectifierAlarm && 
                       rectifierAlarm.trim() !== "" && 
        ...
        if (hasRIF) {
          const coordinates = parseCoordinates(row[17]);
          const region = row[6] || "Unknown";
          const duration = row[13] || "N/A";
          const remainingFuel = parseFloat(row[8]) || 0;
          const siteCategory = row[18] || "Unknown"; // Column S - Site Category/Site Severity
          const siteName = row[0] || "Unknown Site";
          const siteId = row[1] || "N/A";
          const occurTime = row[11] || "N/A"; // Column L - Occur Time
          ...
          siteCategoryCount[siteCategory] = (siteCategoryCount[siteCategory] || 0) + 1;
        }
      }
      ...
      const alarmSitesWithWeather = alarmSites.map(site => {
        const weather = weatherDataMap[site.id] || { risk: "Low", weatherDesc: "Unknown", precipProbability: 0, windSpeed: 0, temperature: null };
      ...
      // Calculate site category stats for the cards
      const categoryStats = {};
      ...
      const badWeatherAffected = alarmSitesWithWeather.filter(s => s.condition === "Bad Weather Affected").length;
      const lowFuelSites = alarmSitesWithWeather.filter(s => s.condition === "Low Fuel (<50L)").length;
      const moderateRiskSites = alarmSitesWithWeather.filter(s => s.condition === "Moderate Risk").length;
      ...
      if (firstValidSite && firstValidSite.coordinates) {
        setMapCenter([firstValidSite.coordinates.lat, firstValidSite.coordinates.lng]);
        setMapZoom(7);
      }
      ...
      // Fallback demo data with ALL 5 site categories
      const demoAlarms = [
        {
          id: "KHI-001",
      ...
      // Calculate category stats for demo data - includes PTN Node now
      ...
      setAlarmStats({
        ...
        badWeatherAffected: demoAlarms.filter(s => s.condition === "Bad Weather Affected").length,
        lowFuelSites: demoAlarms.filter(s => s.condition === "Low Fuel (<50L)").length,
        moderateRiskSites: demoAlarms.filter(s => s.condition === "Moderate Risk").length,
  ...
  // Calculate category stats for the selected region - Show ALL categories including PTN Node
  ...
  const getRegionOptions = () => {
    const regions = ["All", ...new Set(rifAlarms.map(a => a.region).filter(r => r !== "Unknown"))];
    return regions;
  };
  ...
  const navItems = [
    { name: "Dashboard", icon: LayoutDashboard, active: false, onClick: goToDashboard },
   
    { name: "Sites Map", icon: Map, active: false, onClick: goToMap },
    { name: "RIF Alarms", icon: Zap, active: true, onClick: null },
  ];

  // Get category color - Updated to include PTN Node
  const getCategoryColor = (category) => {
    if (category === "PTN Critical" || category.includes("Critical")) {
      return "text-red-400";
    }
    if (category === "PTN Node") {
      return "text-cyan-400";
    }
    if (category.includes("Major") || category.includes("5~10")) {
      return "text-orange-400";
    }
    if (category.includes("Minor") || category.includes("1~4")) {
  ...
  return (
    <div className="h-screen bg-[#0f1325] text-white flex overflow-hidden">
      ...
      >
        ...
        <div className="mb-6">
          <div className="text-xs text-gray-500 uppercase tracking-wider mb-3">Navigation</div>
            ...
            {navItems.map((item, idx) => (
              <button
                ...
                className={`w-full text-left flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all ${
                  item.active ? "bg-red-600/20 text-red-400" : "text-gray-400 hover:bg-[#1a213a]"
                }`}
              >
                <item.icon size={20} />
                <span>{item.name}</span>
        ...
        <div className="mt-auto pt-6 border-t border-gray-800">
          <div className="bg-[#0f1325] rounded-xl p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-xs flex items-center gap-1 ${alarmStats.totalAlarms > 0 ? "text-red-400" : "text-green-400"}`}>
                <Circle size={8} fill={alarmStats.totalAlarms > 0 ? "#ef4444" : "#4ade80"} stroke="none" />
                {alarmStats.totalAlarms > 0 ? `${alarmStats.totalAlarms} Alarms Active` : "All Systems Normal"}
              </span>
            </div>
            <button
          ...
          {isLoading ? (
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p className="text-gray-400">Loading RIF Alarms & Weather Data...</p>
                <p className="text-xs text-gray-500 mt-2">Fetching real-time weather from Open-Meteo</p>
              </div>
            </div>
          ) : (
            <>
              {/* REGION FILTER - Horizontal filter before cards */}
                      ...
                      >
                        ...
                        {region !== "All" && (
                          <span className="ml-2 text-xs opacity-75">
                            ({rifAlarms.filter(a => a.region === region).length})
                          </span>
                        )}
                      </button>
                    ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-gradient-to-br from-red-900/40 to-red-800/20 rounded-2xl border border-red-500/30 p-5">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-gray-300 text-sm">Total Active RIF Alarms</p>
                      <p className="text-3xl font-bold text-white mt-1">{filteredAlarms.length}</p>
                      {selectedRegion !== "All" && (
                        <p className="text-xs text-gray-400 mt-1">in {selectedRegion}</p>
                      )}
                    </div>
                  ...
                  <div className="mt-3 flex items-center gap-2 text-xs">
                    <span className="text-red-400">⚠️ Rectifier Failure Detected</span>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-blue-900/30 to-blue-800/10 rounded-2xl border border-blue-500/20 p-5">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-gray-300 text-sm">Bad Weather Affected</p>
                      <p className="text-3xl font-bold text-blue-400 mt-1">
                        {regionSites.filter(s => s.condition === "Bad Weather Affected").length}
                      </p>
                    </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-orange-400 mt-1">
                        {regionSites.filter(s => s.condition === "Low Fuel (<50L)").length}
                      </p>
                    </div>
                ...
                {/* Site Category Stats Card - Show ALL categories including PTN Node */}
                <div className="bg-gradient-to-br from-purple-900/30 to-purple-800/10 rounded-2xl border border-purple-500/20 p-5">
                  ...
                  <div className="mt-2 space-y-1 max-h-32 overflow-y-auto">
                    {Object.entries(getRegionCategoryStats()).map(([category, stats]) => (
                      <div key={category} className="text-xs flex justify-between items-center">
                        <span className="text-gray-400 truncate max-w-[140px]" title={category}>
                          {category}
                        </span>
                        <span className={getCategoryColor(category)}>{stats.count} sites</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-[#1a2142] to-[#13182b] rounded-2xl border border-gray-700 overflow-hidden">
                  <div className="bg-[#0a0f20] px-4 py-2.5 border-b border-gray-700 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="font-semibold text-white">
                        RIF ALARMS - SITE MAP {selectedRegion !== "All" && `(${selectedRegion})`}
                      </h2>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="text-xs text-red-400">
                        🔴 {filteredAlarms.filter(a => a.coordinates).length} Sites in {selectedRegion !== "All" ? selectedRegion : "All Regions"}
                      </div>
                    </div>
                  </div>
                  <div className="h-[450px] w-full relative bg-[#0f1325]">
                    <MapContainer
                      ...
                      center={mapCenter}
                      zoom={mapZoom}
                      style={{ height: "100%", width: "100%", background: "#0f1325" }}
                      zoomControl={false}
                    ...
                    >
                      ...
                      {filteredAlarms.map((alarm) => (
                        alarm.coordinates && (
                          <Marker
                            key={alarm.id}
                            position={[alarm.coordinates.lat, alarm.coordinates.lng]}
                          ...
                          >
                            <Tooltip sticky direction="top" offset={[0, -15]}>
                              <div className="text-sm font-semibold">{alarm.name}</div>
                              <div className="text-xs flex items-center gap-1 mt-1">
                                {getConditionIcon(alarm.condition)}
                                <span className={alarm.condition === "Bad Weather Affected" ? "text-blue-400" : alarm.condition === "Low Fuel (<50L)" ? "text-orange-400" : "text-yellow-400"}>
                                  {alarm.condition}
                                </span>
                              </div>
                              <div className="text-xs">Fuel: {alarm.remainingFuel.toLocaleString()}L</div>
                              <div className="text-xs">Weather: {alarm.weatherDesc} ({alarm.weatherRisk})</div>
                            </Tooltip>
                            <Popup>
                              <div className="text-black min-w-[260px] p-2">
                                <h4 className="font-bold text-base">{alarm.name}</h4>
                                <p className="text-xs text-gray-500 mt-1">ID: {alarm.siteId}</p>
                                
                                <div className={`mt-2 p-2 rounded text-xs font-medium ${getConditionColor(alarm.condition)}`}>
                                  <div className="flex items-center gap-2">
                                    {getConditionIcon(alarm.condition)}
                                ...
                                <div className="mt-2 space-y-1 text-xs">
                                  <p><strong className="text-gray-700">Alarm:</strong><br/>{alarm.alarmMessage}</p>
                                  <p><strong>Remaining Fuel:</strong> 
                                    <span className={alarm.remainingFuel < 50 ? "text-red-500 font-bold" : ""}>
                                      {" "}{alarm.remainingFuel.toLocaleString()} L
                                    </span>
                                  </p>
                                  <p><strong>Duration:</strong> 
                                    <span className={getDurationColor(alarm.duration)}>
                                      {" "}{formatDuration(alarm.duration)}
                                    </span>
                                  </p>
                                  <p><strong>Occur Time:</strong> {alarm.occurTime || "N/A"}</p>
                                  
                                  {/* Weather Information Section */}
                                  <div className="mt-2 pt-2 border-t border-gray-200">
                                    <p className="font-semibold text-gray-700">🌤️ Weather Information</p>
                                    <p><strong>Condition:</strong> {alarm.weatherDesc}</p>
                                    <p><strong>Risk Level:</strong> 
                                      <span className={alarm.weatherRisk === "Critical" ? "text-red-500 font-bold" : alarm.weatherRisk === "High" ? "text-orange-500" : alarm.weatherRisk === "Moderate" ? "text-yellow-500" : "text-green-500"}>
                                        {" "}{alarm.weatherRisk}
                                      </span>
                                    </p>
                                    <p><strong>Precipitation:</strong> {alarm.precipProbability}%</p>
                                    <p><strong>Wind Speed:</strong> {alarm.windSpeed} km/h</p>
                                    <p><strong>Temperature:</strong> {alarm.temperature}°C</p>
                                  </div>
                                  
                                  <p><strong>Region:</strong> {alarm.region}</p>
                                  <p><strong>Site Category:</strong> 
                                    <span className={getCategoryColor(alarm.siteCategory)}> {alarm.siteCategory}</span>
                                  </p>
                                </div>
                                
                                <button 
                                  onClick={() => {
                                    window.open(`https://www.google.com/maps?q=${alarm.coordinates.lat},${alarm.coordinates.lng}`, '_blank');
                                  }}
                  ...
                  <div className="px-4 py-2 text-[10px] text-gray-400 flex justify-between border-t border-gray-800 bg-[#0a0f20]">
                    <span>🔴 {filteredAlarms.filter(a => a.coordinates).length} sites displayed for {selectedRegion !== "All" ? selectedRegion : "all regions"}</span>
              ...
              {/* ALARMS LIST TABLE - Uses filteredAlarms directly */}
              <div className="bg-gradient-to-br from-[#1a2142] to-[#13182b] rounded-2xl border border-gray-700 overflow-hidden">
                <div className="bg-[#0a0f20] px-5 py-3 border-b border-gray-700 flex justify-between items-center">
                  <div>
                    <h3 className="font-bold flex items-center gap-2">
                      <FileText size={18} className="text-red-400" />
                      ACTIVE RIF ALARMS {selectedRegion !== "All" && `- ${selectedRegion}`}
                    </h3>
                    <p className="text-gray-500 text-xs mt-1">Real-time weather data from Open-Meteo API</p>
                  </div>
                  <button
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-[#0f1325] border-b border-gray-700">
                      <tr>
                        <th className="p-3 text-left text-gray-400 font-medium text-xs">Site Name</th>
                        <th className="p-3 text-left text-gray-400 font-medium text-xs">Region</th>
                        <th className="p-3 text-left text-gray-400 font-medium text-xs">Duration</th>
                        <th className="p-3 text-left text-gray-400 font-medium text-xs">Occur Time</th>
                        <th className="p-3 text-left text-gray-400 font-medium text-xs">Weather</th>
                        <th className="p-3 text-left text-gray-400 font-medium text-xs">Alarm Message</th>
                        <th className="p-3 text-right text-gray-400 font-medium text-xs">Remaining Fuel</th>
                        <th className="p-3 text-left text-gray-400 font-medium text-xs">Category</th>
                        <th className="p-3 text-center text-gray-400 font-medium text-xs">Weather Risk</th>
                      </tr>
                    </thead>
                      ...
                      {filteredAlarms.length === 0 ? (
                        <tr>
                          <td colSpan="9" className="p-8 text-center text-gray-400">
                            ...
                            <p>No active RIF alarms found for {selectedRegion !== "All" ? selectedRegion : "this region"}</p>
                            <p className="text-xs mt-1">All rectifiers are operating normally</p>
                          </td>
                        </tr>
                      ) : (
                        filteredAlarms.map((alarm) => (
                          <tr
                            ...
                            onClick={() => {
                              if (alarm.coordinates) {
                                setMapCenter([alarm.coordinates.lat, alarm.coordinates.lng]);
                                setMapZoom(14);
                          ...
                          >
                            <td className="p-3 text-white font-medium">
                              {alarm.name}
                              <p className="text-xs text-gray-500">{alarm.siteId}</p>
                            </td>
                            <td className="p-3 text-gray-300">{alarm.region}</td>
                            <td className="p-3">
                              <div className="flex items-center gap-2">
                                <Hourglass size={14} className={getDurationColor(alarm.duration)} />
                                <span className={`text-sm font-mono ${getDurationColor(alarm.duration)}`}>
                                  {formatDuration(alarm.duration)}
                                </span>
                              </div>
                            </td>
                            <td className="p-3 text-gray-300 text-xs">{alarm.occurTime || "N/A"}</td>
                            <td className="p-3">
                              <div className="flex items-center gap-1">
                                {alarm.weatherRisk === "Critical" ? (
                                  <CloudRain size={14} className="text-red-400" />
                                ) : alarm.weatherRisk === "High" ? (
                                  <CloudRain size={14} className="text-orange-400" />
                                ) : alarm.weatherRisk === "Moderate" ? (
                                  <CloudRain size={14} className="text-yellow-400" />
                                ) : (
                                  <CloudRain size={14} className="text-green-400" />
                                )}
                                <span className="text-xs text-gray-300">{alarm.weatherDesc}</span>
                              </div>
                            </td>
                            <td className="p-3 text-gray-300 max-w-xs truncate" title={alarm.alarmMessage}>
                              {alarm.alarmMessage.length > 40 ? `${alarm.alarmMessage.substring(0, 40)}...` : alarm.alarmMessage}
                            </td>
                            <td className="p-3 text-right">
                              <span className={alarm.remainingFuel < 50 ? "text-red-400 font-bold text-base" : "text-gray-300"}>
                                {alarm.remainingFuel.toLocaleString()} L
                              </span>
                            ...
                            <td className="p-3">
                              <span className={`text-xs px-2 py-1 rounded-full ${getCategoryColor(alarm.siteCategory)} bg-opacity-10`}>
                                {alarm.siteCategory}
                              </span>
                            </td>
                            <td className="p-3 text-center">
                              <div className="flex items-center justify-center gap-1">
                                {alarm.weatherRisk === "Critical" || alarm.weatherRisk === "High" ? (
                                  <>
                                    <CloudRain size={14} className="text-red-400" />
